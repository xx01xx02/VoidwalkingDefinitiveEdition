...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['lmao get good', 0.2], //From 0% to 19%
		['extremely bad', 0.4], //From 20% to 39%
		['bad', 0.5], //From 40% to 49%
		['bruh', 0.6], //From 50% to 59%
		['its alright', 0.69], //From 60% to 68%
		['sex number joke', 0.7], //69%
		['still shit tbh', 0.8], //From 70% to 79%
		['better', 0.9], //From 80% to 89%
		['based', 1], //From 90% to 99%
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'train':
				...
				trainback = new FlxBackdrop(Paths.image('trainback'));
				add(trainback);
				trainback.velocity.set(75, 0);

				traininside = new BGSprite('traininside', 0, -25, 0.9, 0.9);
				traininside.updateHitbox();

			case 'void':
				var bg:BGSprite = new BGSprite('maxhomebg', -600, -200, 0.9, 0.9);
				add(bg);

				var stageFront:BGSprite = new BGSprite('maxhomestage', -600, -200, 0.9, 0.9);
				stageFront.setGraphicSize(Std.int(stageFront.width * 1));
				stageFront.updateHitbox();
				add(stageFront);

			case 'phobia':
				var bg:BGSprite = new BGSprite('maxvoidbg', -600, -200, 0.9, 0.9);
				add(bg);

				var stageFront:BGSprite = new BGSprite('maxvoidstage', -600, -200, 0.9, 0.9);
				stageFront.setGraphicSize(Std.int(stageFront.width * 1));
				stageFront.updateHitbox();
				add(stageFront);

			case 'petrol':
				var mountains:BGSprite = new BGSprite('bgmountains', -600, -200, 0.9, 0.9);
				add(mountains);
				var orb:BGSprite = new BGSprite('bgtheorb', -600, -200, 0.9, 0.9);
				add(orb);
				var floor:BGSprite = new BGSprite('bgfloor', -600, -200, 0.9, 0.9);
				add(floor);

			case 'rhr':
				var bg:BGSprite = new BGSprite('rhrbg', -600, -200, 0.9, 0.9);
				add(bg);
			...
			case 'jim':
				var bg:BGSprite = new BGSprite('JIMMYHOME', -600, -200, 0.9, 0.9);
				add(bg);

			case 'thej':
				defaultCamZoom = 0.85;
				curStage = 'thej';
				var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('sphere'));
				...
				var stageFront:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('cliff'));
	...
	public function endSong():Void
	{
		...
		if(achievementObj != null) {
			...
			var achieve:String = checkForAchievement(['week1_done', 'week1_nomiss', 'week2_nomiss', 'week3_nomiss', 'week4_nomiss',
